...
const AddQuestions = ({addQuestion}) => {
    ...
    const runFunc = () =>{
        ...
        if(!option1 || !option2 || !option3 || !option4) {alert("WRITE ALL THE OPTIONS"); flag=1}
        if(!flag){
        addQuestion({question , option1,option2,option3,option4,correctOption})
        }
    }
    ...
    return (
        <div>
            <form>
                ...
                <input value={option4} onChange={(e) => setOption4(e.target.value)} placeholder='option 4'></input>
                <input value={correctOption} onChange={(e) => setCorrectOption(e.target.value)} placeholder='
                (small case a/b/c/d)'></input>
